...
def generate(lm: LoadedModel, prompt: str, max_new_tokens: int = 64) -> tuple[str, list[float]]:
...
def generate_batch(
    ...
    # === Prefill ===
    # Explicitly pass DynamicCache so transformers 5.x doesn't pre-allocate a
    # StaticCache for the full context window (32K tokens × batch_size = OOM).
    ...
    all_ids = torch.cat([input_ids, next_ids], dim=1)
    attn_mask = torch.cat([attn_mask, torch.ones(batch_size, 1, device=lm.device)], dim=1)
